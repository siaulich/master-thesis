...
\node[dnn_block, fill=green!20,right=1.5cm of jet_1, minimum height = 0.8cm] (jet_dnn_1) {Jet DNN};
\node[dnn_block, fill=green!20,right=1.5cm of jet_n, minimum height = 0.8cm] (jet_dnn_n) {Jet DNN};
\node[yshift=0.05cm] at ($(jet_dnn_1)!0.5!(jet_dnn_n)$) (dnn_jet_dots) {$\vdots$};

% Input Embeddings for leptons
\node[dnn_block, fill=cyan!20, right=1.5cm of lepton_1, minimum height = 0.8cm] (lep_dnn_1) {Lep DNN};
\node[dnn_block, fill=cyan!20, right=1.5cm of lepton_2, minimum height = 0.8cm] (lep_dnn_2) {Lep DNN};

% Input Embeddings for met
\node[dnn_block, fill=red!20,right=1.5cm of met, minimum height = 0.8cm] (met_dnn) {MET DNN};
...
\path let \p1 = (jet_dnn_1.north), \p2 = (met_dnn.south) in
    node[transformer_layer,
             right=1.5cm of transformer_mid,
