...
% Jet Encoder
\coordinate (jet_midpoint) at ($(jet_dnn_1.east)!0.5!(jet_dnn_n.east)$);

\path let \p1 = (jet_dnn_1.north), \p2 = (jet_dnn_n.south) in
    node[self_attention,
             right=0.8cm of jet_midpoint.east,
             minimum height={\y1-\y2},
             minimum width=1cm]
             (transformer_1) {\rotatebox{90}{Self-Attention}};
...
% Lepton Encoder
\coordinate (lepton_midpoint) at ($(lep_dnn_1.east)!0.5!(lep_dnn_2.east)$);

\path let \p1 = (lep_dnn_1.north), \p2 = (lep_dnn_2.south) in
    node[self_attention,
             right=0.8cm of lepton_midpoint,
             minimum height={\y1-\y2},
             minimum width=1cm]
             (lep_transformer_1) {\rotatebox{90}{Self-Attention}};
...
\node[output] at ($(cross_attention_probs.east |- jet_1)+(1.5,0)$) (jet_1_prob_b) {$P(b)$};
\node[output] at ($(cross_attention_probs.east |- jet_n)+(1.5,0)$) (jet_n_prob_b) {$P(b)$};
\node[yshift=0.05cm] at ($(jet_1_prob_b)!0.5!(jet_n_prob_b)$) (output_b_dots) {$\vdots$};

\node[output, right=0.5cm of jet_1_prob_b] (output_bbar_1) {$P(\bar{b})$};
\node[output, right=0.5cm  of jet_n_prob_b] (output_bbar_n) {$P(\bar{b})$};
...
\node[fill=white, inner sep = 0.1cm, outer sep = 0] at ($(jet_1)+(1.25,0)$) (conact_jet_1) {$\bigoplus$};
\node[fill=white, inner sep = 0.1cm, outer sep = 0] at ($(jet_n)+(1.25,0)$) (conact_jet_n) {$\bigoplus$};
\node[fill=white, inner sep = 0.1cm, outer sep = 0] at ($(lepton_1)+(1.25,0)$) (conact_lep_1) {$\bigoplus$};
\node[fill=white, inner sep = 0.1cm, outer sep = 0] at ($(lepton_2)+(1.25,0)$) (conact_lep_2) {$\bigoplus$};
...
\draw[arrow] (jet_dnn_1.east -| cross_attention_probs.east) -- (jet_1_prob_b.west);
\draw[arrow] (jet_dnn_n.east -| cross_attention_probs.east) -- (jet_n_prob_b.west);
...
\node[fit=(jet_1_prob_b)(jet_n_prob_b)(output_bbar_1)(output_bbar_n), 
      draw, 
      inner sep=0.2cm, 
