...
\cleardoublepage
\begin{abstract}
  \noindent Many of the recent highlights in particle physics research are related to top-quark physics. These include both the stringent tests of spin correlations and quantum effects in pairs of top quarks ($t\bar{t}$), and the observation of a possible quasi-bound-state resonance in the $t\bar{t}$ invariant mass spectrum. The latter presents a very exciting opportunity to extend the current understanding of non-relativistic Quantum Chromodynamics. Both effects are predominantly studied in the dilepton decay channel of the top quark in the mass range close to the $t\bar{t}$ production threshold.

  The dilepton final state contains two neutrinos, besides two charged leptons and two $b$-quark jets. Probing this system requires a precise reconstruction of the top quarks, which is complicated by the presence of the two neutrinos.  While past reconstruction strategies primarily focused on inferring the neutrino momenta, the problem of correctly assigning $b$-jets to their parent top quarks remained largely understudied. However, many of the sensitive variables used in $t\bar{t}$  precision measurements depend critically on the correct assignment of the jets.

  Inspired by the success of machine learning architectures in tackling the assignment challenge for hadronic decay channels, as well as tools that provide neutrino regression for the dilepton channel, this work demonstrates how both tasks can be performed by a single model. For this purpose, various models and setups are implemented and evaluated. A discussion of machine-learning-related aspects of the different architectures is followed by an in-depth analysis of the method's strengths and merits. Lastly, using a simplified analysis setup, its ability to improve measurement sensitivity will be demonstrated. By comparing the distributions of \atlas event data and simulated events, the model's readiness for use in a full analysis setting is demonstrated. The method presented here is the very first ML tool to provide a full reconstruction of the dileptonic decay channel that is available in the \atlas-experiment's official software repository. Applying the tools developed in this work to existing and upcoming analyses promises substantial enhancements in the sensitivity and precision of searches and measurements alike.
  \bigskip\par
