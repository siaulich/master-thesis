...
As mentioned before, supervised training requires labels associated with the input data. While this can be a major bottleneck for many machine learning applications, it is largely absent in particle physics. In \Cref{sec:mc_colliders}, it was elaborated on how fundamental physics is simulated at colliders. Events are sampled from the underlying parton distributions and then further processed to match the observed detector-level structure. Thus, the necessary labels that the model seeks to predict are readily available for simulated events.
...
\begin{equation}
R(f) = \mathbb{E}{\mathbf{x}, \mathbf{y}}[L(\mathbf{y},   f\theta(\mathbf{x}))]
\end{equation}
...
\sum_{i=1}^{N} (y_i - f_\theta(\mathbf{x}i))^2
\end{equation}
...
Cross-attention is permutation-equivariant with respect to permutations of the query inputs and separately to permutations of the key/value inputs.
