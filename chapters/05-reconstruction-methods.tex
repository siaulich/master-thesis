...
This section provides a brief overview on the modelling and reconstruction of top-quark pair events. The simulated sampled of \ttbar-events is generated using a Monte-Carlo event generation as described in \Cref{sec:mc_training_data}.

While a complete analysis requires detailed investigation of background processes contributing to the signal regions, the following Chapters focus on developing and evaluating machine-learning methods applied to \ttbar events.
Therefore, only the signal process of \ttbar production with dileptonic decays is described here. The background processes and their simulation is discussed in the following \Cref{chap:physics-analysis}, which investigates the tool's performance in a physics-analysis.

\subsection{Simulation of Dileptonic Top-Quark Pair Events}
\label{sec:nominal_ttbar}
The signal process of \ttbar production with dileptonic decays is simulated using \powheg~\cite{powheg} at next-to-leading order (NLO) in perturbative quantum chromodynamics (QCD). The \powheg generator is interfaced with \pythia~\cite{pythia} for parton showering, hadronization, and underlying event modelling. A more detailed description can be found in Reference \cite{atlas_toponium}. The samples used here are created with the hvq-model.
