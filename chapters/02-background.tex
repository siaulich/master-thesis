...
Bosons are integer-spin gauge fields mediating the fundamental forces: the photon for electromagnetism, the $W^\pm$ and $Z$ bosons for the weak interaction, and eight gluons for the strong interaction.
...
\begin{itemize}
  ...
  \item \textbf{Parton showers}, which resum logarithmically enhanced     emissions and are implemented in Monte Carlo generators such as \pythia,     first developed in~\cite{SJOSTRAND1985321,parton_shower} and     extended in modern formulations~\cite{pythia}.
...
\end{itemize}
These aspects of QCD are essential for reconstructing hadronic final states and for Monte Carlo simulations of \lhc physics.
...
\begin{figure}
  ...
  \label{fig:mc_stages}
\end{figure}
The complex mathematical structure of the standard model, combined with the different scales that are involved in the processes at colliders, which means analytical predictions are rarely feasible.
Therefore, predictions are obtained almost exclusively via Monte Carlo (MC) simulations of the underlying physical processes. For this, event samples are generated by randomly sampling from the partons' probability distributions.

Events are usually simulated using a cascade of different programs, each simulating a different state of the event. First, the hard-scattering process is simulated using matrix-element generators.
These programs calculate the probabilities of different particle interactions based on underlying theories of physics, such as the Standard Model. Using these probabilities, they generate events representing the particles' initial state after the collision. The possible kinematic phase space is sampled according to these probabilities, resulting in a set of particles with specific momenta and energies. This type of event variable is often referred to as \textit{parton-level}.
Next, the parton showering and hadronisation processes are simulated.
Parton showering models the emission of additional particles from the initial partons, while hadronisation simulates the formation of hadrons from quarks and gluons. These processes are crucial for accurately modelling the final state particles observed in detectors.

Finally, detector simulation programs are used to model particle interactions with the detector material, producing realistic detector responses. This includes simulating the energy deposits in calorimeters, hits in tracking detectors, and other relevant signals.
While the actual detector response is simulated with complex detector simulation software, for many applications a simplified representation is sufficient. This can involve smearing particle momenta and energies to match the detector resolution, applying efficiency corrections, and simulating the effects of pile-up.
...
\begin{figure}[t]
  ...
  \caption{Leading order Feynman diagrams for top-quark pair production in \subrefb{fig:top_production_qq_s_channel}
    quark-antiquark annihilation and \subrefb{fig:top_production_gg_t_channel}, \subrefb{fig:top_production_gg_s_channel} \& \subrefb{fig:top_production_gg_u_channel} gluon fusion (ggF).}
...
One particular region of interest is the so-called \emph{threshold region}, where the top-quark pair is produced with low relative velocity. In this region, the top quark is produced as a spin singlet state in about $\sim 80\%$ of the cases, leading to a strong entanglement signature~\cite{toponium_2}. The \atlas collaboration has recently reported the first experimental evidence of quantum correlations consistent with entangled spin quantum states in top-quark pairs~\cite{atlas_entanglement}. Measurements of this variable are particularly sensitive in the dilepton channel, because the lepton has the highest spin-analysing power~\cite{spin_analyzing_power,spin_analyzing_power_jets}. The spin-analysing power measures the extent to which the spin information of the parent is imprinted in the angular distributions of the decay products. Due to QCD effects, this is lower for light jets compared to leptons.
...
To compute the variables, top quarks and leptons are boosted to the rest frame of the \ttbar system.
...
\end{equation}
Analogously, one can define the variable \chan, where either of the lepton momenta components parallel to the parent top-quark momentum in the \ttbar rest frame is flipped before taking the inner product \cite{bernreuther_nlospin_2004,bernreuther_spin_observables}. This variable adds important context by enabling isolation of scalar states. Differential cross-sections of the spin-states obtained from simulation are found in \Cref{fig:spin_sensitive_theory}. The distributions clearly show the distinctive shapes of the different spin states. Further, a stark difference between the slope of the perturbative QCD computation and the modelling of the quasi-bound state can be found. This highlights the variables' sensitivity to the spin structure of the \ttbar system.
