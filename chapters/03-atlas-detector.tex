...
The \atlas detector is a general-purpose particle detector at the \lhc at \cern. With its length of $46$ m and diameter of $25$ m, it is the largest detector ever built at a particle collider.
The cylindrical detector consists of three primary elements: the inner detector, the calorimeter, and the muon spectrometer~\cite{atlas_tdr}. These components are arranged concentrically around the beam pipe, with each system enclosing the previous one.
Tracks of charged particles are bent using an extensive system of magnets. 
...
It is used for tracking charged particles, particle identification, and primary- and secondary-vertex determination~\cite{atlas_tdr}. The ID consists of three different sub-detectors, in addition to the Insertable B-Layer, which was installed after \LHCruni \cite{atlas_ibl_tdr}. Their rough structure is depicted in \Cref{fig:inner_det}. The magnetic field is crucial to measure the momentum of a charged particle since the transverse momentum of a charged particle traversing this magnetic field is proportional to the curvature of the particle's trajectory.
...
The following part of the detector is the semiconductor tracker (SCT). It consists of $4088$ modules arranged in four layers to ensure four-position measurements of charged particles. Each module consists of four silicon sensors. The sensors offer a $17\,\unit{\micro \metre}$ resolution in-plane laterally and $580\,\unit{\micro \metre}$ in-plane longitudinally.
...
The transition radiation tracker utilises the transition radiation emitted by charged particles traversing the interface between two media with different refractive indices. The TRT provides velocity measurements of charged particles, which allows for electron identification.
